...
1. Let's add on a join to one final table - `organisation`. This will add details of the organisation at which someone held a post to what our previous query returned - and that will give us a full picture of the appointments people have carried out.
    ...
    **Challenge**: Can you adapt that statement to give:
    ...
    - All appointments since May 1979 in the Cabinet Office?
    - All appointments since May 1979 that lasted less than 30 days? Subtraction between two columns of dates will give you the difference in number of days.[^7] (Note that appointments in the database are considered to end where a post has moved from one department to another - so some of the records you return here, such as Michael Gove's appointment as Secretary of State for Housing, Communities and Local Government, in reality lasted more than 30 days. Linking up appointments that span these ['machinery of government changes'](https://www.instituteforgovernment.org.uk/explainer/machinery-government-changes) is possible but beyond the scope of this tutorial)
...
1. Combining things you've learnt, it's also possible to use `count` in combination with joins.
    ...
    **Challenge**: Using what you learnt earlier:
    - Can you adapt the query to work out how different the gender balance in appointments looks if we restrict things to appointments since 3 May 1997 (the date of the 1997 general election), or between 6 May 2010 and 4 July 2024 (the 2010 and 2024 general elections)?[^8]
    - Returning to our Smiths, can you calculate tallies of the departments they have held appointments in?[^9]
...
[^7]: To do this you'll need a `where` clause with `a.end_date - a.start_date < 30` or similar.
[^8]: This will need a `where` clause, making use of `start_date`, or `start_date` and `end_date`, in the `appointment` table.
[^9]: This will also need a `where` clause using `ilike`, as you employed earlier, to select people whose `short_name` ends with 'Smith' or 'smith'. And, rather than grouping on `gender` from the `person` table, you'll need to group on something from the `organisation` table.
