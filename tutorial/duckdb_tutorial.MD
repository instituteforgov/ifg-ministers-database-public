...
1. At its heart, the database is about tracking which individuals did which posts for which time period. In order to do this, we need to use the `appointment` table, which links people and posts.
    ...
    - This gives `t.name` an alias of `post_name` in the results (see the `select` part of the query) - this isn't strictly needed, but helps make clear what the column is when we have another column called `name`
