...
For our purposes, a browser-based version, making use of WebAssembly, is also available. Files in various formats, including CSV, JSON and Parquet, can be read in and queried as if they form a database - as well as .
...
## Tutorial
### Reading the data in
1. We [publish the database on GitHub](https://github.com/instituteforgov/ifg-ministers-database-public/) under a Creative Commons by-attribution licence.
\
\
If you look in the `data` folder you should see multiple CSV files - one for each table in the database - and one SQLite data. For the purposes of this tutorial, we'll use the [SQLite file](https://github.com/instituteforgov/ifg-ministers-database-public/blob/main/data/ifg-ministers-database.sqlite).
\
The database consists of 11 tables, but for the purposes of this tutorial we'll work with five of them:
    ...
    - `appointment_characteristics`: Each `appointment` has one or more corresponding `appointment_characteristics` records

Files can be read directly in to DuckDB with minimal effort. To read in these five CSV files and turn them into tables with the relevant name, run the following:
...
- `id`: An identified tied to a unique individual, which we'll use for joining together records across tables
...
[^2]: The database uses gender rather than sex, as we draw that particular variable from Parliamentary inforation, and that is the term used there.
...
TODO: If you have some experience using SQL you might want to skip ahead at this stage to step XXX.
...
This returns all `rank_equivalence` values in the `post` table. `rank_equivalence` is an IfG-derived categorisation which attempts to classify posts with posts of similar seniority. The values are as follows:
- PM: Prime minister
...
- Parl. lead.: A parliamentary leadership role
- Parl.: Another parliamentary role, such as a government whip
...
Two other features to flag: this gives `t.name` an alias of `post_name` in the results - this isn't strictly needed, but helps make clear what the column is when we have another column called `name`. And `coalesce()` is used on the join between the `person` table and the `appointment` table. This has the effect of (temporarily) replacing numbers in the columns it is being called on with '`1900-01-01`' or '`9999-12-31`', respectively - dates outside the range present for any of our appointments. This ensures that, where a `person` record doesn't have a `start_date` or an `end_date` that isn't going to prevent us matching any appointments for that individual.[^1]

[^1]: Comparing `a.start_date` to `p.start_date` and `p.end_date` picks up someone's person characteristics - name and gender - at the point where they _start_ an appointment.

This code snippet can be tweaked to give slightly different results. Can you adapt the code to get details of the first 30 appointments since May 1979 of rank Secretary of State?. See the footnotes if you need some pointers.[^1]

[^1]: To do this you'll need a `where` clause using the `rank_equivalence` column in the `post` table.

1. Let's add on a join to one final table - `organisation` - then we'll have a f
...
- All current appointments?[^1]
- All appointments since May 1979 in the Cabinet Office?[^1]
- All appointments since May 1979 that lasted less than 30 days? (Subtraction between two columns of dates will give you the difference in number of days. Note that appointments in the database are considered to end where a post has moved from one department to another - so some of the records you return here, such as Michael Gove's appointment as Secretary of State for Housing, Communities and Local Government in reality lasted more than 30 days. Linking up appointments that span these ['machinery of government changes'](https://www.instituteforgovernment.org.uk/explainer/machinery-government-changes) is possible but beyond the scope of this tutorial)[^1]

Again, see the footnotes for hints.

[^1]: Ongoing appointments have an `end_date` of `NULL`. Rather than using `= null` we use `is null`.
[^1]: To do this you'll need to make use of `start_date` and `end_date` from the `appointment` table.
[^1]: To do this you'll need a `where` clause with `a.end_date - a.start_date < 30` or similar.
...
- How many appointments since May 1979 have been held by women and how many have been held by men?[^1]
- How about appointments just since 3 May 1997?[^1]

[^1]: To do this you'll need a `where` clause using the `gender` column in the `person` table.
[^1]: This will need an additional `where` clause, making use of `start_date` in the `appointment` table.
