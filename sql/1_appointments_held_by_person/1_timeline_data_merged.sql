WITH ministerial_appointment(organisation_id, organisation_short_name, rank_equivalence, appointment_characteristics_start_date, appointment_characteristics_end_date) AS
         (SELECT GROUP_CONCAT(CAST(q.organisation_id AS varchar(36)), '/') organisation_id,
          ...
          FROM (SELECT ROW_NUMBER() OVER (PARTITION BY person_id, appointment_characteristics_id ORDER BY continues_previous_appointment DESC, group_name) ROW_NUMBER,
                       *
                FROM (SELECT CASE
...
FROM ministerial_appointment ma1
    ...
    NOT EXISTS (SELECT *
                ...
                WHERE ma2.appointment_characteristics_end_date >= ma3.appointment_characteristics_start_date
                  AND ma2.appointment_characteristics_end_date < ma3.appointment_characteristics_end_date
                  AND ma2.organisation_id = ma3.organisation_id
                  AND ma2.rank_equivalence = ma3.rank_equivalence)
WHERE NOT EXISTS (SELECT *
                  FROM ministerial_appointment ma4
                  WHERE ma1.appointment_characteristics_start_date > ma4.appointment_characteristics_start_date
                    AND ma1.appointment_characteristics_start_date <= ma4.appointment_characteristics_end_date
                    AND ma1.organisation_id = ma4.organisation_id
                    AND ma1.rank_equivalence = ma4.rank_equivalence)
GROUP BY ma1.organisation_id,
         ma1.organisation_short_name,
         ma1.rank_equivalence,
         ma1.appointment_characteristics_start_date
ORDER BY ma1.appointment_characteristics_start_date
